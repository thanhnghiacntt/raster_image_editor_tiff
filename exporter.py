...
def crs_to_wkt(crs):
    """QgsCoordinateReferenceSystem -> WKT mà GDAL hiểu chắc chắn.

    Ưu tiên mã chuẩn (EPSG:xxxx) để GDAL tự tra định nghĩa đầy đủ; hệ tự định
    nghĩa không có mã thì dùng WKT do QGIS sinh ra.
    """
    ...
    if authid and not authid.upper().startswith('USER:'):
        try:
            if srs.SetFromUserInput(authid) == 0:
                return srs.ExportToWkt()
        except RuntimeError:
            pass
    try:
        wkt = crs.toWkt(QgsCoordinateReferenceSystem.WKT_PREFERRED_GDAL)
...
def export_geotiff(image, placement, crs, out_path,
                   ...
                   world_file=False, dst_crs=None):
    """Xuất `image` (QImage) đã căn theo `placement` ra GeoTIFF tại `out_path`.

    - `crs`: hệ tọa độ mà `placement` đang dùng (hệ của khung bản đồ).
    - `dst_crs`: hệ tọa độ của file đầu ra (vd. EPSG:4326, EPSG:3857). Bỏ trống
      hoặc trùng `crs` thì giữ nguyên. Khác `crs` thì ảnh được chiếu lại.
    - `north_up`: nếu ảnh có góc xoay, nắn lại thành ảnh thẳng hướng Bắc
      ...
      GeoTransform của GeoTIFF (không mất chất lượng). Khi đổi hệ tọa độ thì
      luôn phải nắn lại, tham số này không còn tác dụng.
    ...
    reproject = (dst_crs is not None and dst_crs.isValid()
                 and crs is not None and crs.isValid() and dst_crs != crs)
    dst_wkt = crs_to_wkt(dst_crs) if reproject else wkt
    ...
    try:
        if reproject or (north_up and rotated):
